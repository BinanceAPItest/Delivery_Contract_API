...
toc_footers:
  - <a href='https://www.binance.com/cn/futures/'>Binance Futures</a>
...
* 新增接口 `POST /fapi/v1/s/countdownCancelAll` 以实现倒计时自动撤单。   
	* 该接口可以被用于确保在发生故障时撤销指定symbol上的所有挂单。 
...
--- 
<font size=4>**2020-1-17**</font>

* Python [SDK和代码示例](#sdk) 发布


---

<font size=4>**2020-1-6**</font>
...
* 访问 [https://github.com/Binance-docs/Binance_Futures_python](https://github.com/Binance-docs/Binance_Futures_python)
...
可以通过以下方式获取SDK：   
* 访问 [https://github.com/Binance-docs/Binance_Futures_Java](https://github.com/Binance-docs/Binance_Futures_Java),   
...
```shell
    (HMAC SHA256)
    $ curl -H "X-MBX-APIKEY: vmPUZE6mv9SD5VNHk4HlWFsOr6aKE2zvsw0MuIgwCIPy6utIco14y7Ju91duEh8A" -X POST 'https://fapi.binance.com/fapi/v1/order?symbol=LTCBTC&side=BUY&type=LIMIT&timeInForce=GTC&quantity=1&price=0.1&recvWindow=5000&timestamp=1499827319559&signature=c8db56825ae71d6d79447849e617115f4a920fa2acdcab2b053c4b2838bd6b71'
```
...
```shell
    (HMAC SHA256)
    $ curl -H "X-MBX-APIKEY: vmPUZE6mv9SD5VNHk4HlWFsOr6aKE2zvsw0MuIgwCIPy6utIco14y7Ju91duEh8A" -X POST 'https://fapi.binance.com/fapi/v1/order?symbol=LTCBTC&side=BUY&type=LIMIT&timeInForce=GTC' -d 'quantity=1&price=0.1&recvWindow=5000&timestamp=1499827319559&signature=0fd168b8ddb4876a0358a8d14d0c9f3da0e9b20c5d52b2a00fcf7d1c602f9a77'
```
...
{
    "symbol": "BTCUSDT",				// 交易对
    "markPrice": "11012.80409769",		// 标记价格
    "lastFundingRate": "-0.03750000",	// 最近更新的资金费率
    "nextFundingTime": 1562569200000,	// 下次资金费时间
...
* 本篇所列出的所有wss接口的baseurl为: **wss://fstream.binance.com**
* 订阅单一stream格式为 **/ws/\<streamName\>**
* 组合streams的URL格式为 **/stream?streams=\<streamName1\>/\<streamName2\>/\<streamName3\>**
* 订阅组合streams时，事件payload会以这样的格式封装 **{"stream":"\<streamName\>","data":\<rawPayload\>}**
* stream名称中所有交易对均为**小写**
* 每个到**fstream.binance.com**的链接有效期不超过24小时，请妥善处理断线重连。
...
type | INT | YES | 1: 现货账户向合约账户划转 2:  合约账户向现货账户划转
recvWindow | LONG | NO  | 
timestamp | LONG | YES
...
[
 	{
 		...
 		"asset": "USDT",		// 资产
 		"balance": "122607.35137903",	// 余额
 		"withdrawAvailable": "102333.54137903", // 最大可提款金额,同`GET /fapi/account`中“maxWithdrawAmount”
	}
]
```
...

{
	"assets": [	// 资产内容
	 ...
	 "feeTier": 2, // 手续费等级
	 "maxWithdrawAmount": "8.41264592", // 最大可提款金额总额
	 "positions": [ // 头寸
		 {
...
* 本篇所列出的websocket接口baseurl: **wss://fstream.binance.com**
...
"Filter failure: MARKET_LOT_SIZE" | `MARKET`市价订单里`quantity`太高或者太低。通过`/fapi/v1/exchangeInfo`接口可以拿到相关信息。
